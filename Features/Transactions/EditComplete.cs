...
namespace Piggyzen.Web.Features.Transaction
{
    public class EditComplete
    {
        ...
        public class CommandHandler : IRequestHandler<Command>
        {
            ...
            {
                var client = _httpClientFactory.CreateClient("Api");
                var response = await client.PutAsJsonAsync($"transaction/complete-update/{request.Id}", request, cancellationToken);

                if (!response.IsSuccessStatusCode)
                {
                    throw new InvalidOperationException("Failed to update complete transaction.");
                }
            }
        }
    }
