...
namespace Piggyzen.Web.Features.Transaction
{
    public class CreateComplete
    {
        ...
        public class CommandHandler : IRequestHandler<Command>
        {
            ...
            {
                var client = _httpClientFactory.CreateClient("Api");
                var response = await client.PostAsJsonAsync("transaction/create-complete", request, cancellationToken);

                if (!response.IsSuccessStatusCode)
                {
                    throw new HttpRequestException("Failed to create complete transaction.");
                }
            }
        }
    }
